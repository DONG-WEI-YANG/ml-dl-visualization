...
> 此預設帳號密碼僅供明確設定 `APP_ENV=development` 的本機環境。首次登入必須先修改密碼，後端才允許其他功能。正式環境需獨立 `JWT_SECRET`（至少 32 字元）及初始管理員密碼（至少 12 字元），設定不足會拒絕啟動。

2026-09-20 安全修復新增可撤銷 session、可信評分流程與 18 週基本題庫。部署前請閱讀 [資料保存與遷移操作](docs/storage-operations.md)；舊 token 將失效，需要重新登入。題庫初始化只補缺漏，不覆寫既有題目。
