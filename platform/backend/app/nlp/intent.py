...
INTENT_PATTERNS = {
    ...
    "how": [
        (r"怎麼做", 1.0), (r"如何實作", 1.0), (r"怎樣寫", 0.9), (r"步驟", 1.0),
    ...
    "compare": [
        ...
        (r"取捨", 1.0), (r"怎麼選", 1.0), (r"哪個好", 1.0), (r"哪個比較", 1.0),
        (r"該用哪", 1.0), (r"差在哪", 1.0), (r"哪個適合", 0.9), (r"搞混", 0.8),
        (r"什麼時候用哪", 1.0), (r"用哪個", 0.9), (r"跟.*差", 0.8),
        (r"vs", 1.0), (r"versus", 1.0), (r"difference", 1.0), (r"compare", 1.0),
        (r"which.+better", 1.0), (r"trade.?off", 1.0), (r"when to use which", 0.9),
    ],
    "example": [
        (r"舉例", 1.0), (r"例子", 1.0), (r"範例", 1.0), (r"示範", 0.9),
        (r"實例", 0.9), (r"案例", 0.8), (r"demo", 1.0),
        (r"example", 1.0), (r"show me", 0.9), (r"demonstrate", 0.9),
    ],
    "debug": [
        (r"錯誤", 0.9), (r"報錯", 1.0), (r"失敗", 0.8), (r"異常", 0.9),
        (r"當掉", 1.0), (r"噴錯", 1.0), (r"出error", 1.0), (r"結果不對", 0.9),
        (r"跑出來.*奇怪", 0.9), (r"全部.*一樣", 0.8), (r"是0$", 0.7),
        (r"nan", 1.0), (r"inf", 1.0), (r"一直是", 0.7),
    ...
    "formula": [
        ...
        (r"方程", 1.0), (r"證明", 0.9), (r"微分", 1.0), (r"偏微分", 1.0),
        (r"導數", 1.0), (r"積分", 0.9), (r"怎麼算", 0.8),
        (r"formula", 1.0), (r"equation", 1.0), (r"derive", 1.0), (r"proof", 0.9),
        (r"derivative", 1.0), (r"gradient.*計算", 0.9),
    ],
    "code": [
    ...
    "parameter": [
        ...
        (r"怎麼設", 0.9), (r"預設值", 0.8), (r"設太大", 1.0), (r"設太小", 1.0),
        (r"設多少", 1.0), (r"夠不夠", 0.8), (r"要設幾", 0.9), (r"要跑幾", 0.9),
        (r"要多少才", 0.9), (r"通常設", 0.9), (r"預設", 0.7),
        (r"會不會太大", 0.9), (r"會不會太小", 0.9), (r"設成.*會", 0.8),
        (r"parameter", 1.0), (r"hyperparameter", 1.0), (r"tuning", 0.9),
        (r"how many epochs", 0.9), (r"what.*set.*to", 0.8),
    ],
    "performance": [
    ...
    "application": [
        ...
        (r"產業", 0.9), (r"業界", 0.9), (r"在.+怎麼用", 1.0), (r"可以.*做什麼", 0.8),
        (r"醫療", 0.8), (r"金融", 0.8), (r"製造", 0.8), (r"工業", 0.8),
        (r"application", 1.0), (r"real.world", 1.0), (r"use case", 1.0),
    ],
    "prerequisite": [
        (r"先備", 1.0), (r"需要先", 0.9), (r"基礎", 0.8), (r"先學", 1.0),
        (r"先會", 1.0), (r"之前要.*會", 0.9), (r"之前要.*學", 0.9),
    ...
    "troubleshoot": [
        ...
        (r"版本", 0.8), (r"相容", 0.9), (r"衝突", 0.9), (r"跑不起來", 1.0),
        (r"開不起來", 1.0), (r"啟動不了", 1.0), (r"連不上", 0.9),
        (r"pip", 0.9), (r"conda", 0.9), (r"套件", 0.7), (r"GPU偵測", 1.0),
        (r"CUDA", 1.0), (r"PATH", 0.9),
        (r"install", 1.0), (r"setup", 0.9), (r"version", 0.8), (r"dependency", 0.9),
        (r"cannot import", 1.0), (r"module not found", 1.0),
    ],
    "deeper": [
