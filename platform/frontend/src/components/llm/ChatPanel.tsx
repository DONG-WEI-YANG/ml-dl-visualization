...
export default function ChatPanel({ week, topic }: Props) {
  ...
  if (collapsed) {
    return (
      <button
        ...
        className="w-full flex items-center justify-between px-4 py-3 border border-gray-200 rounded-xl bg-white hover:bg-gray-50 transition-colors group cursor-pointer"
      >
        <div className="flex items-center gap-2.5 pointer-events-none">
          <span className="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center text-sm font-bold group-hover:bg-blue-100 transition-colors">
            AI
          </span>
          <div className="text-left">
            <span className="text-sm font-medium text-gray-900">AI 助教</span>
            <span className="text-xs text-gray-400 ml-2">點擊展開對話</span>
          </div>
        </div>
        <svg className="w-4 h-4 text-gray-400 group-hover:text-gray-600 transition-colors pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  ...
  return (
    <div
      className={`flex flex-col border border-gray-200 rounded-xl bg-white transition-all ${
        pinned ? "h-[calc(100vh-120px)] max-h-[700px] min-h-[400px]" : "h-[420px]"
      }`}
      role="region"
    ...
    >
      ...
      {/* Input */}
      <form onSubmit={handleSubmit} className="flex gap-2 p-2.5 border-t border-gray-200">
        <input
          value={input}
