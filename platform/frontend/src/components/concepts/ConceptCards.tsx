...
const WEEK_CONCEPTS: Record<number, ConceptCard[]> = {
  1: [
    {
      title: "Python 環境管理",
      description: "使用 venv 或 conda 建立隔離的開發環境，避免套件版本衝突。",
      keyPoints: [
        "虛擬環境讓每個專案擁有獨立的套件版本",
        "pip freeze 可匯出 requirements.txt 方便重現環境",
        "conda 同時管理 Python 版本和系統級依賴",
      ],
    },
    {
      title: "NumPy 陣列運算",
      description: "NumPy 提供高效的多維陣列運算，是科學計算的基礎。",
      keyPoints: [
        "ndarray 支援向量化運算，速度遠快於 Python list",
        "廣播機制讓不同形狀的陣列可以直接運算",
        "reshape、索引和切片是最常用的操作",
      ],
    },
    {
      title: "Pandas DataFrame",
      description: "Pandas 是資料處理的核心工具，DataFrame 像是程式化的 Excel 表格。",
      keyPoints: [
        "read_csv / read_excel 可快速載入資料",
        "loc 和 iloc 分別以標籤和位置進行索引",
        "groupby 配合聚合函式可快速統計資料",
      ],
    },
    {
      title: "Jupyter Notebook",
      description: "互動式開發環境，可以逐步執行程式碼並即時看到結果。",
      keyPoints: [
        "Cell 可以是程式碼、Markdown 或 Raw 格式",
        "Shift+Enter 執行當前 Cell 並跳到下一個",
        "魔術指令如 %timeit 可以測量執行時間",
      ],
    },
    {
      title: "套件生態系",
      description: "Python ML/DL 生態系包含 scikit-learn、TensorFlow、PyTorch 等核心套件。",
      keyPoints: [
        "scikit-learn 適合傳統機器學習",
        "Matplotlib 和 Seaborn 用於資料視覺化",
        "pip install 是最常見的安裝方式",
      ],
    },
  ],
  2: [
